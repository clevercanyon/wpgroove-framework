...
trait Transient_Members {
	 ...
	 *
	 *                         * This parameter does not exist in WordPress core {@see get_transient()}.
	 *
	 * @return mixed {@see get_transient()}, {@see U\Str::maybe_unserialize()}.
	 *
	 *               * Unlike {@see get_transient()}, this returns value with its original data type.
	 *                 Made possible by {@see U\Str::serialize()}, {@see U\Str::maybe_unserialize()}.
	 *
	 *               * Unlike {@see get_transient()}, this returns `null` (or `$default`) on miss or failure.
	 *                 Made possible by {@see U\Str::serialize()}, {@see U\Str::maybe_unserialize()}.
	 */
	final public function get_transient( /* mixed */ $key_parts, /* mixed */ $default = null ) /* : mixed */ {
		assert( ! is_resource( $key_parts ) );
		assert( ! $key_parts instanceof \Closure );

		$key   = U\Crypto::sha1_key( $key_parts );
		$value = get_transient( $this->var_prefix . $key );
	 ...
	 *
	 *                          * Passing `null` explicitly will {@see delete_transient()}.
	 *
	 *                          * Unlike {@see set_transient()}, this stores values w/ their original data type.
	 *                            Made possible by {@see U\Str::serialize()}, {@see U\Str::maybe_unserialize()}.
	 *
	 *                          * PHP does not allow a {@see \Closure} to be serialized whatsoever.
	 ...
	 */
	final public function set_transient( /* mixed */ $key_parts, /* mixed */ $value, int $expires_in = U\Time::HOUR_IN_SECONDS ) : bool {
		assert( ! is_resource( $key_parts ) );
		assert( ! $key_parts instanceof \Closure );
		...
		if ( null === $value ) {
			return $this->delete_transient( $key_parts );
		}
		$key        = U\Crypto::sha1_key( $key_parts );
		$value      = U\Str::serialize( $value );
		$expires_in = $expires_in <= 0 ? U\Time::HOUR_IN_SECONDS : $expires_in;

		return set_transient( $this->var_prefix . $key, $value, $expires_in );
	}
	 ...
	 */
	final public function delete_transient( /* mixed */ $key_parts ) : bool {
		assert( ! is_resource( $key_parts ) );
		assert( ! $key_parts instanceof \Closure );

		$key = U\Crypto::sha1_key( $key_parts );
		return delete_transient( $this->var_prefix . $key );
	}
}
