...
class Post_Update_Cmd_Handler extends \Clever_Canyon\Utilities\OOP\Abstracts\A6t_CLI_Tool {
	...
	protected function maybe_compile_wp_plugin_svn_repo() : void {
		...
		if ( 0 !== U\CLI::run( [ 'php', '../../vendor/varunsridharan/wp-cli-textdomain/src/add-text-domain.php', '-i', $plugin->headers->text_domain, U\Dir::join( $plugin_svn_comp_dir, '/vendor/clevercanyon/wpgroove-framework' ) ], $plugin_svn_comp_dir, false ) ) {
			throw new Exception( 'Failed to run `php ../../vendor/varunsridharan/wp-cli-textdomain/src/add-text-domain.php -i ' . $plugin->headers->text_domain . ' ' . U\Dir::join( $plugin_svn_comp_dir, '/vendor/clevercanyon/wpgroove-framework' ) . '` in ./._x/svn-comp directory.' );
		}
	...
	protected function maybe_compile_wp_theme_svn_repo() : void {
		...
		if ( 0 !== U\CLI::run( [ 'php', '../../vendor/varunsridharan/wp-cli-textdomain/src/add-text-domain.php', '-i', $theme->headers->text_domain, U\Dir::join( $theme_svn_comp_dir, '/vendor/clevercanyon/wpgroove-framework' ) ], $theme_svn_comp_dir, false ) ) {
			throw new Exception( 'Failed to run `php ../../vendor/varunsridharan/wp-cli-textdomain/src/add-text-domain.php -i ' . $theme->headers->text_domain . ' ' . U\Dir::join( $theme_svn_comp_dir, '/vendor/clevercanyon/wpgroove-framework' ) . '` in ./._x/svn-comp directory.' );
		}
