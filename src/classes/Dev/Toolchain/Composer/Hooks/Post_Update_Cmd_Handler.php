...
class Post_Update_Cmd_Handler extends \Clever_Canyon\Utilities\OOP\Abstracts\A6t_CLI_Tool {
	...
	protected function maybe_compile_wp_plugin_svn_repo() : void {
		...
		if ( 0 !== U\CLI::run( [ '../../vendor/bin/php-scoper', 'add-prefix', '--config', U\Dir::join( $plugin_svn_comp_dir, '/.scoper.cfg.php' ), '--prefix', ucfirst( $this->project->name_hash ), '--no-interaction', '--force', '--output-dir', $plugin_svn_distro_dir ], $plugin_svn_comp_dir, false ) ) {
	...
	protected function maybe_compile_wp_theme_svn_repo() : void {
		...
		if ( 0 !== U\CLI::run( [ '../../vendor/bin/php-scoper', 'add-prefix', '--config', U\Dir::join( $theme_svn_comp_dir, '/.scoper.cfg.php' ), '--prefix', ucfirst( $this->project->name_hash ), '--no-interaction', '--force', '--output-dir', $theme_svn_distro_dir ], $theme_svn_comp_dir, false ) ) {
